function out = forceEvaluation(t,y,state,imap,icheck,nUpdate,nRadius,msIndex)
	persistent F;
	persistent fcount;
	persistent ltime;
	persistent ptime;
	...
	F_bending = 0;
	F_spring = 0;
	F_bottom_substrate = 0;
	F_load = 0;
	F_moving_substrate = 0;
	F_atom = 0;
	...
	for j = 1:m
		for i = 1:n(j)
			...
			F_bending = F_bending - beta*xout;
			F_bending = F_bending - beta*yout;
			%}
			...
			F_spring = F_spring - gamma*(forward_x + backward_x);
			F_spring = F_spring - gamma*(forward_y + backward_y);
			%}
			
			% V.W. /w Substrate
			F_bottom_substrate = F_bottom_substrate - LJsubstrate(y(yI_out),epsi,sigma,density);
	...
	for h = 1:length(F)
		...
		if i < 0
			...
			F_moving_substrate = F_moving_substrate - LJval*temp_x;
			F_moving_substrate = F_moving_substrate - LJval*temp_y;
			F_atom = F_atom + LJval*temp_x;
			F_atom = F_atom + LJval*temp_y;
		else
		%}
			xps = y(i) - y(j);
			yps = y(i+N) - y(j+N);
	...
	out = [ F_bending, F_spring, F_bottom_substrate, F_load, ...
		F_moving_substrate, F_atom ];
